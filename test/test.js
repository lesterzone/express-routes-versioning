...
describe('routes versioning', function() {
  ...
  it('if version is not provided by client, ' +
    ...
    function() {
      ...
      assert.ok(NoMatchFoundSpy.calledWith(request, response, next));
    });
  ...
  it('if version if not provided by client, ' +
    ...
    function() {
      ...
      assert.ok(latestVersionSpy.calledWith(request, response, next));
    });

  it('if accept-version header is presponseent, ' +
    ...
    function() {
      ...
      request.headers = {};
      request.headers['accept-version'] = '1.2.1';
      middleware(request, response, next);
      assert.ok(version1Spy.calledOnce);
      assert.ok(version1Spy.calledWith(request, response, next));
    });
  ...
  it('when multiple version are provided, ' +
    ...
    function() {
      ...
      request.version = '2.3.1';
      middleware(request, response, next);
      assert.ok(version2Spy.calledOnce);
      assert.ok(version2Spy.calledWith(request, response, next));
      assert.ok(version1Spy.neverCalledWith(request, response, next));
    });
